...
os.chdir('C:\\Users\\gdavi\\Desktop\\Aplicativo')

planilhas=pd.ExcelFile('C:\\Users\\gdavi\\Desktop\\Aplicativo\\real.xlsx')
...
for criança in crianças:
    if criança in atendR.index:
        ...
        INCONSISTENCIAS.append('ERRO 0 - A criança '+str(criança)+' não consta nas planilhas "Atendimento Regular" e "Atendimento Esporádico"')
...
for row in compilado.index:
    if compilado.loc[row]['Tratamentos'] != 0:
        ...
        if quant_atend >= tamanho_dispo:
            INCONSISTENCIAS.append('ERRO 1 - A criança'+str(row)+'apresenta mais demanda de tratamentos do que horários disponíveis')
...
def ChecaRestr(criança,hora,tratamento,funcionario):
    if criança == 'Indisponível':
        ...
        INCONSISTENCIAS.append('Erro 3 - A criança '+str(criança)+' não consta na planilha de atendimentos')
    else:
        #Lógica da Nutrição
...
for kid in compilado1.index: 
    if kid not in df_c_atend_OR.index:
        INCONSISTENCIAS.append('AVISO - A '+str(kid) + ' não foi atendida')
        Cria_n_atend.append(kid)
    elif type(df_c_atend_OR.loc[kid]['Horário'])== np.int64:
        if sum(compilado1.loc[kid]['Tratamentos'].values()) != 1:
            if 'Nutrição' in compilado1.loc[kid]['Tratamentos'].keys():
                if compilado1.loc[kid]['Frequencia']['Nutrição'] == 'Quinzenal':
                    aqwee=10
            else:    
                INCONSISTENCIAS.append('AVISO -A criança ' + str(kid) + ' não teve todos seus atendimentos alocados')
                Cria_n_atend.append(kid)
    else:
...
wb=openpyxl.load_workbook('C:\\Users\\gdavi\\Desktop\\Aplicativo\\real.xlsx',keep_vba=True)
...
wb.save('modelo-de-dados_ccp_ResultPEKA.xlsm')
